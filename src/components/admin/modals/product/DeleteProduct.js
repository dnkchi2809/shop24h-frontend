...
function DeleteProduct(props) {
    ...
    const { openDeleteProductModal } = useSelector((reduxData) => reduxData.reducers);
            ...
            .then((data) => {
                if (data.status == "Success 204") {
                    dispatch({
                        ...
                        payload: {
                            openSnackbar: true,
                            alertString: "Delete product succesfully"
                        }
                    })
                    dispatch({
                        type: "ALERT_SEVERITY",
    ...
    return (
        <>
            <Modal open={openDeleteProductModal} onClose={handleModalClose}>
                <Box style={style}>
                    <Container>
                        ...
                        <Row className="mt-2 text-center">
                            {
                                props.product !== null
                                    ?
                                    <Typography>Delete productname = {props.product.name} ?</Typography>
                                    :
                                    null
                            }
